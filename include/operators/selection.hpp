  ...
  namespace operators{
    ...
    class Selection
    {
      public:
        ...
        void do_selection(vector<vector<int>> &parents, vector<double> &parents_fitness, const vector<vector<int>> &offspring, const vector<double> &offspring_fitness)
        {
          switch (this->selection_operator_)
          {
          case 1:
            this->elistism_plus(parents, parents_fitness, offspring, offspring_fitness);
            break;
          case 2:
            this->elistism_comma(parents, parents_fitness, offspring, offspring_fitness);
            break;
          case 3:
            this->tournament_plus(parents, parents_fitness, offspring, offspring_fitness);
            break;
          case 4:
            this->tournament_comma(parents, parents_fitness, offspring, offspring_fitness);
            break;
          case 5:
            this->proportion_plus(parents, parents_fitness, offspring, offspring_fitness);
            break;
          case 6:
            this->proportion_comma(parents, parents_fitness, offspring, offspring_fitness);
            break;
          default:
        ...
        /// \fn elistism_comma
        /// @brief Select the best solutions from the offspring population.
        /// @param parents
        /// @param parents_fitness
        /// @param offspring
        /// @param offspring_fitness
        void elistism_comma(vector<vector<int>> & parents, vector<double> & parents_fitness, const vector<vector<int>> &offspring, const vector<double> &offspring_fitness)
        {
        ...
        /// \fn elistism_plus
        /// @brief Select the best solutions from the parent and offspring population.
        /// @param parents
        /// @param parents_fitness
        /// @param offspring
        /// @param offspring_fitness
        void elistism_plus(vector<vector<int>> & parents, vector<double> & parents_fitness, const vector<vector<int>> &offspring, const vector<double> &offspring_fitness)
        {
          assert(parents.size() == parents_fitness.size());
          assert(offspring.size() == offspring_fitness.size());
        ...
        /// \fn tournament_comma
        /// @brief Tournament selection for the best solutions from the offspring population.
        /// @param parents
        /// @param parents_fitness
        /// @param offspring
        /// @param offspring_fitness
        void tournament_comma(vector<vector<int>> & parents, vector<double> & parents_fitness, const vector<vector<int>> &offspring, const vector<double> &offspring_fitness, const size_t tournament_k_ = 2)
        {
          size_t mu = parents_fitness.size();
          size_t lambda = offspring_fitness.size();

          assert(tournament_k_ <= lambda);
          ...
          for (size_t i = 0; i != mu; ++i)
          {
            sample_n_from_m(sample_k, tournament_k_, lambda);
            if (Opt == optimizationType::MAXIMIZATION)
            {
              tmp_best = numeric_limits<double>::lowest();
              for (size_t j = 0; j != tournament_k_; ++j)
              {
        ...
        /// \fn tournament_plus
        /// @brief Tournament selection for the best solutions from the parent and offspring population.
        /// @param parents
        /// @param parents_fitness
        /// @param offspring
        /// @param offspring_fitness
        void tournament_plus(vector<vector<int>> & parents, vector<double> & parents_fitness, const vector<vector<int>> &offspring, const vector<double> &offspring_fitness, const size_t tournament_k_ = 2)
        {
          ...
          assert(tournament_k_ <= lambda + mu);
          ...
          for (int i = 0; i != mu; ++i)
          {
            sample_n_from_m(sample_k, tournament_k_, mu + lambda);
            if (Opt == optimizationType::MAXIMIZATION)
            {
              tmp_best = numeric_limits<double>::lowest();
              for (size_t j = 0; j != tournament_k_; ++j)
        ...
        /// \fn proportion_comma
        /// @brief Proportionala selection for the best solutions from the offspring population.
        /// @param parents
        /// @param parents_fitness
        /// @param offspring
        /// @param offspring_fitness
        void proportion_comma(vector<vector<int>> & parents, vector<double> & parents_fitness, const vector<vector<int>> &offspring, const vector<double> &offspring_fitness)
        {
          size_t mu = parents_fitness.size();
          size_t lambda = offspring_fitness.size();
        ...
        /// \fn proportion_plus
        /// @brief Proportionala selection for the best solutions from the parent and offspring population.
        /// @param parents
        /// @param parents_fitness
        /// @param offspring
        /// @param offspring_fitness
        void proportion_plus(vector<vector<int>> & parents, vector<double> & parents_fitness, const vector<vector<int>> &offspring, const vector<double> &offspring_fitness)
        {
          vector<vector<int>> backup_parents = parents;
          vector<double> backup_parents_fitness = parents_fitness;
          size_t mu = parents_fitness.size();
